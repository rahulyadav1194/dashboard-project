...
useEffect(()=>{
    ...
    async function getData(){
        let result = await axios.get('http://localhost:700/api/getCustomer')
        setData(result.data)
    }

    async function deleteData(id){
        let result = confirm('Are U sure data delete')
        if(result == true){
            await axios.delete(`http://localhost:700/api/deleteCustomer/${id}`)
            getData()
        }
                  ...
                  <tbody className="divide-y divide-gray-200 bg-white">
                    {data.map((data) => (
                      ...
                      <tr >
                        ...
                        class="focus:outline-none text-white bg-yellow-400 hover:bg-yellow-500 focus:ring-4 focus:ring-yellow-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:focus:ring-yellow-900">Update</Link>
